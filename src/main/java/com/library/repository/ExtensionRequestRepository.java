...
public interface ExtensionRequestRepository extends JpaRepository<ExtensionRequest, Long> {
    ...
    @Query("""
        ...
        AND (:borrower IS NULL OR br.borrower = :borrower)
        AND (:fromDate IS NULL OR er.createdAt >= :fromDate)
        AND (:toDate IS NULL OR er.createdAt <= :toDate)
        ORDER BY 
            CASE er.status
